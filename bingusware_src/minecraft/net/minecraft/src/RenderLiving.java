...
public class RenderLiving extends Render
{
    ...
    public void doRenderLiving(EntityLiving entity, double x, double d0, double z, float entityYaw, float partialTicks)
    {
        ...
        try
        {
            ...
            float var12 = entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks;
            this.renderLivingAt(entity, x, d0, z);
            float var13 = this.handleRotationFloat(entity, partialTicks);
            this.rotateCorpse(entity, var13, var10, partialTicks);
            float var14 = 0.0625F;
            GL11.glEnable(GL12.GL_RESCALE_NORMAL);
            GL11.glScalef(-1.0F, -1.0F, 1.0F);
            this.preRenderCallback(entity, partialTicks);
            GL11.glTranslatef(0.0F, -24.0F * var14 - 0.0078125F, 0.0F);
            float var15 = entity.prevLimbYaw + (entity.limbYaw - entity.prevLimbYaw) * partialTicks;
            float var16 = entity.limbSwing - entity.limbYaw * (1.0F - partialTicks);
            ...
            this.mainModel.setLivingAnimations(entity, var16, var15, partialTicks);
            ...
            {
                var18 = this.shouldRenderPass(entity, var17, partialTicks);

                if (var18 > 0)
                {
                    this.renderPassModel.setLivingAnimations(entity, var16, var15, partialTicks);
                    this.renderPassModel.render(entity, var16, var15, var13, var11 - var10, var12, var14);

                    if ((var18 & 240) == 16)
                    {
                        this.func_82408_c(entity, var17, partialTicks);
                        this.renderPassModel.render(entity, var16, var15, var13, var11 - var10, var12, var14);
                    }

                    if ((var18 & 15) == 15)
                    {
                        var19 = (float)entity.ticksExisted + partialTicks;
            ...
            float var26 = entity.getBrightness(partialTicks);
            var18 = this.getColorMultiplier(entity, var26, partialTicks);
            ...
            if ((var18 >> 24 & 255) > 0 || entity.hurtTime > 0 || entity.deathTime > 0)
            {
                ...
                if (entity.hurtTime > 0 || entity.deathTime > 0)
                {
                    ...
                    {
                        if (this.inheritRenderPass(entity, var27, partialTicks) >= 0)
                        {
                            GL11.glColor4f(var26, 0.0F, 0.0F, 0.4F);
                            this.renderPassModel.render(entity, var16, var15, var13, var11 - var10, var12, var14);
                        }
                ...
                if ((var18 >> 24 & 255) > 0)
                {
                    ...
                    {
                        if (this.inheritRenderPass(entity, var28, partialTicks) >= 0)
                        {
                            GL11.glColor4f(var19, var20, var29, var22);
                            this.renderPassModel.render(entity, var16, var15, var13, var11 - var10, var12, var14);
                        }
        ...
        GL11.glPopMatrix();
        this.passSpecialRender(entity, x, d0, z);
    }

    /**
     * Renders the model in RenderLiving
     */
    protected void renderModel(EntityLiving entityLiving, float par2, float par3, float par4, float par5, float par6, float par7)
    {
        this.callLoadTextureFunction(entityLiving);

        // What baffles me is that bat.png is 64x64, and it loads just fine, but newer skins just don't...?
//      this.loadTexture("/mob/bat.png");

        if (!entityLiving.isInvisible())
        {
            this.mainModel.render(entityLiving, par2, par3, par4, par5, par6, par7);
        }
        // i think this is unreachable (i think? i dont know so i wont touch it. (even tho i wanna))
        else if (!entityLiving.func_98034_c(Minecraft.getMinecraft().thePlayer))
        {
            GL11.glPushMatrix();
        ...
        {
            this.mainModel.setRotationAngles(par2, par3, par4, par5, par6, par7, entityLiving);
        }
    }

    protected void callLoadTextureFunction(EntityLiving par1EntityLiving)
    {
