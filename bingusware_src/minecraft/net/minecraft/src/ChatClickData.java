...
/**
 * @edited 7/20/21
 * @purpose Make it easier to access screenshots
 */
public class ChatClickData {
        ...
        if (var1 != null) {
            try {
                ...
                if (var2.matches()) {
                    ...
                    return new URI(var3);
                } else if (!this.line.getChatLineString().contains(var1)) {
                    return new URI("file:///" + var1);
                }
    ...
    private String findClickedUrl() {
        String screenshotsDir = Minecraft.getMinecraftDir().toURI().getRawPath();
        screenshotsDir = screenshotsDir.substring(0, screenshotsDir.length() - 2) + "screenshots/";

        if (this.line.getChatLineString().contains(Utils.screenshotUrlDesign))
            return screenshotsDir + this.line.getChatLineString().split(Utils.screenshotUrlDesign)[1];
